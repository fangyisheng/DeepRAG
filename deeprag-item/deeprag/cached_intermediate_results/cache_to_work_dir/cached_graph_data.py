...
async def cache_entity_relation(entity_relation_by_chunk, graph_data):
    data = {
        ...
        "text_chunk_id": [f"text_chunk_{str(uuid.uuid4())}" for _ in range(0,len(entity_relation_by_chunk))],
        "graph_data": graph_data,
        "graph_data_id": [f"graph_data_{str(uuid.uuid4())}" for _ in range(0,len(entity_relation_by_chunk))]
    }
