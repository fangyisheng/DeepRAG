...
async def realize_leiden_community_algorithm(graph_data):
    ...
    # 使用 PyVis 可视化
    net = Network(notebook = True, cdn_resources="in_line", height="750px", width="100%",bgcolor="#222222", font_color="white",directed=True)
    net.set_options("""
    {
    "physics": {
        "enabled": true,
    ...
    for community in community_membership:
         community_color[community] = f"#{random.randint(0, 255):02x}{random.randint(0, 255):02x}{random.randint(0, 255):02x}"
   
    for i, community in enumerate(community_membership):
        
        color = community_color[community]
        community = community_membership[i]
        net.nodes[i]["label"]  = f"{net.nodes[i]["text"]} ({net.nodes[i]["type"]})"  # 设置节点标签
