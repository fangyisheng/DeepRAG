...
class IndexWorkFlowDAO:
    ...
    async def create_workflow(
        ...
        async with Prisma() as db:
            stored_workflow = await db.index_workflow.create(
                data={
                    "id": id,
                    "status": status,
                    "action": action,
    ...
    async def get_workflow_by_id(self, id: str) -> index_workflow:
        async with Prisma() as db:
            found_workflow = await db.index_workflow.find_unique(where={"id": id})

        return found_workflow

    async def get_workflow_by_message_id(self, message_id: str) -> index_workflow:
        async with Prisma() as db:
            found_workflow = await db.index_workflow.find_unique(
    ...
    async def update_workflow(self, id: str, data: dict) -> index_workflow:
        async with Prisma() as db:
            ...
            updated_workflow = await db.index_workflow.update(
                where={"id": id},
                data=data,
            )
