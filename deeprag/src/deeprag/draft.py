...
async def worker(delay):
    ...
    return f"任务完成，耗时 {delay:.2f} 秒"


async def main():
    """使用 as_completed 和 tqdm 显示任务进度"""
    total_tasks = 10
    tasks = [worker(random.uniform(0.5, 2)) for _ in range(total_tasks)]

    print("开始执行任务...")
    for future in tqdm_asyncio(
        asyncio.as_completed(tasks), total=total_tasks, desc="test"
    ):
        result = await future
        print(f"结果: {result}")
    return "所有任务完成！"
